...
def test_search_library_success(mock_response):
    with patch("requests.get") as mock_get:
        ...
        result = search_library("test", "vpl", {"vpl": "http://test.com?query={query}"}, {"vpl": "http://test.com"})
...
def test_search_library_invalid_key():
    result = search_library("test", "invalid", {"vpl": "http://test.com?query={query}"}, {"vpl": "http://test.com"})
    assert "error" in result
